...
@Slf4j
@Component
public class SubjectAnswerMenuMsgHandle implements WeixinMsgHandle {
  ...
  public boolean canHandle(EventMessage msg) {
    ...
    if (msg.getMsgType().equals(MsgType.EVENT.getValue())&&msg.getEvent().equals(EventType.CLICK.getValue()) ) {
      if (MenuAnswerEnum.convertEnum(msg.getEventKey())==null){
        return false;
      }
  ...
  public XMLMessage handleMsg(EventMessage msg) throws Exception {
    ...
    answer.setAnswer(MenuAnswerEnum.convertEnum(msg.getEventKey()).getAnswer());
    Subject subject = subjectRepository.findOne(subjectId);

    log.debug("找到主题:{}",subject);
    if (answer.getAnswer().equals(subject.getRightAnswer())) {
      // 回答正确
